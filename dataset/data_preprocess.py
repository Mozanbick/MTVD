...
INSTRUCTION_TEMPLATE = {
    "vul_pre": "Detect whether the following code contains vulnerabilities.",
    "dep_pre": "Output whether there are dependencies between the code entities.",
    "fix_pre": "Judge whether the input code function is patched."
}


def gen_instruction_dataset(dataset):
    src_db = os.path.join(os.getcwd(), dataset)

    def add_instruction(src, dst):
        with open(os.path.join(src_db, src), 'r') as fp:
            samples = [json.loads(line) for line in fp.readlines()]
        new_samples = []
        for sam in samples:
            item = {
                "instruction": INSTRUCTION_TEMPLATE['vul_pre'],
                "input": sam['func'],
                "output": sam['target'],
                "idx": sam['idx']
            }
            new_samples.append(item)
        with open(os.path.join(src_db, dst), 'w') as fp:
            for ns in new_samples:
                fp.write(json.dumps(ns) + '\n')
    
    add_instruction('train.jsonl', 'train_mt.jsonl')
    add_instruction('valid.jsonl', 'valid_mt.jsonl')
    add_instruction('test.jsonl', 'test_mt.jsonl')
